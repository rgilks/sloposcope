...
class SlopTester:
    ...
    def run_test(self, test_case: SlopTestCase) -> dict:
        ...
        try:
            ...
            cmd = [
                sys.executable, "-m", "sloplint.cli",
                "analyze", "/tmp/test_text.txt",
    ...
    def analyze_results(self, results: list[dict]) -> dict:
        ...
        for expected_level in ["high", "low"]:
            level_results = [r for r in results if r.get("expected") == expected_level and r.get("success", False)]
            if level_results:
                correct = sum(1 for r in level_results if self._matches_expected_level(r))
    ...
    def _identify_improvement_areas(self, results: list[dict]) -> list[str]:
        ...
        high_results = [r for r in results if r.get("expected") == "high" and r.get("success", False)]
        low_results = [r for r in results if r.get("expected") == "low" and r.get("success", False)]

        high_accuracy = sum(1 for r in high_results if self._matches_expected_level(r)) / len(high_results) if high_results else 0
        low_accuracy = sum(1 for r in low_results if self._matches_expected_level(r)) / len(low_results) if low_results else 0
        ...
        misclassified = [r for r in results if r.get("success", False) and not self._matches_expected_level(r)]
...
def main():
    ...
    for result in results:
        if result.get("success", False):
            ...
            print(f"{status} {result['name']}: {score_formatted} ({result['actual_level']})")
            if result.get("key_issues"):
                print(f"    Issues: {', '.join(result['key_issues'][:3])}")
        else:
            print(f"❌ {result['name']}: ERROR - {result.get('error', 'Unknown error')}")
