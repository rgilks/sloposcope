...
class UltimateSlopDetectorV4:
    """Ultimate slop detector v4 with balanced detection."""
    ...
    def _analyze_repetition(self, text: str) -> Dict[str, Any]:
        """Balanced repetition analysis that avoids false positives."""
        ...
        repeated_words = {
            ...
            if count > 2 and len(word) > 3  # Higher threshold to avoid false positives
        }

        # Sentence similarity with balanced threshold
        sentence_words = [
        ...
        for i in range(len(sentence_words)):
            for j in range(i + 1, len(sentence_words)):
                if len(sentence_words[i]) > 3 and len(sentence_words[j]) > 3:
                    ...
                    if similarity > 0.6:  # Balanced threshold
                        similar_sentences += 1
    ...
    def _is_natural_human_writing(
        ...
        """Check if text appears to be natural human writing to avoid false positives."""
        # Natural human writing indicators
        natural_indicators = [
            r"\b(I\'ve|I have|I was|I am|I\'m|I can|I will|I would|I should|I might|I may)\b",
            r"\b(honestly|frankly|actually|really|truly|genuinely|personally)\b",
            r"\b(months|years|days|weeks|hours|minutes|ago|yesterday|today|tomorrow)\b",
            r"\b(working on|been working|started|finished|completed|failed|succeeded)\b",
            r"\b(challenging|difficult|hard|easy|simple|complex|complicated)\b",
        ]
        ...
        # If there are natural indicators and low slop scores, it's likely human writing
        if (
            natural_score > 2
            and slop_indicators.get("templates", 0) < 5
            and slop_indicators.get("buzzwords", 0) < 5
        ):
            return True
    ...
    def detect_slop(self, text: str) -> Dict[str, Any]:
        """Detect slop using balanced thresholds and scoring."""
        ...
        # Check for natural human writing to avoid false positives
        ...
        # BALANCED THRESHOLD - adjust based on natural writing detection
        base_threshold = 0.025
        threshold = (
            base_threshold * 2 if is_natural else base_threshold
        )  # Higher threshold for natural writing

        is_slop = slop_score > threshold
        confidence = min(slop_score * 8, 1.0)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Ultimate AI Slop Detection Tool v4",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python ultimate_slop_detector_v4.py "This is high-quality technical content."
  python ultimate_slop_detector_v4.py --file document.txt
  python ultimate_slop_detector_v4.py --json "AI-generated slop text here"
  python ultimate_slop_detector_v4.py --test  # Run test cases
        """,
    )
    ...
    # Initialize detector
    detector = UltimateSlopDetectorV4()

    if args.test:
        # Run test cases
        print("🧪 Ultimate Slop Detection v4 Test Cases")
        print("=" * 60)
        ...
        print(
            f"🎯 Ultimate v4 detector shows {'excellent' if accuracy >= 95 else 'good' if accuracy >= 90 else 'moderate'} performance"
        )
    ...
    try:
        ...
        if args.json:
            ...
            print("\n🔍 Ultimate Slop Detection v4 Analysis")
            print("=" * 50)
