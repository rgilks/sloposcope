...
class LearnedCombiner:
    ...
    def __init__(
        self, model_type: str = "logistic_regression", domain: str = "general"
    ):
    ...
    def fit(self, feature_dicts: list[dict[str, Any]], labels: list[int]) -> None:
        ...
        if len(feature_dicts) != len(labels):
            raise ValueError(
                "Number of feature dictionaries must match number of labels"
            )
        ...
        # Log performance
        logger.info(
            f"Learned combiner ({self.model_type}) fitted on {len(X_train)} samples"
        )
...
class EnsembleDetector:
    ...
    def predict(self, feature_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "confidence": abs(combiner_prob - 0.5)
            * 2,  # Higher confidence when prediction is more certain
        }
